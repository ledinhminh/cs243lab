...
/**
 * Skeleton class for implementing a reaching definition analysis using the
 * Flow.Analysis interface.
 */
public class ReachingDefs implements Flow.Analysis {

    /**
     * Class for the dataflow objects in the ReachingDefs analysis. You are free
     * to change this class or move it to another file.
     */
    ...
    public static class MyDataflowObject implements Flow.DataflowObject {
        ...
        public void genDef(int id) {
            set.add(id);
        }

        public void killDef(int id) {
            set.remove(id);
        }
    }
        ...
        while (qit.hasNext()) {
          ...
          for(RegisterOperand def : q.getDefinedRegisters()){
            ...
            if (!map.containsKey(v)) {
                map.put(v, new TreeSet<Integer>());
            }
            map.get(v).add(q.getID());
            }
        }
    ...
    public void processQuad(Quad q) {
        ...
        val.copy(in[q.getID()]);
        for (RegisterOperand def : q.getDefinedRegisters()) {
            for(int id : map.get(def.getRegister().toString())){
                val.killDef(id);
            }
            val.genDef(q.getID());
        }
