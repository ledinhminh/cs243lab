...
public class ConstantProp implements Flow.Analysis {
	...
	public static class ConstantPropTable implements Flow.DataflowObject {
		...
		/*
		 * 'core' is used to keep track of which variables we need to track
		 */
	...
	public static class TransferFunction extends QuadVisitor.EmptyVisitor {
		...
		public void visitInstanceOf(Quad q) {
			String key = Operator.InstanceOf.getDest(q).getRegister()
					.toString();
			val.setNAC(key);
		}
		...
		private boolean isUndef(Operand op) {
			return (op instanceof RegisterOperand && val.get(
					((RegisterOperand) op).getRegister().toString()).isUndef());
		}

		private boolean isConst(Operand op) {
			return (op instanceof IConstOperand)
					|| (op instanceof RegisterOperand && val.get(
							((RegisterOperand) op).getRegister().toString())
							.isConst());
		}

		private boolean isNAC(Operand op) {
			return (op instanceof RegisterOperand && val.get(
					((RegisterOperand) op).getRegister().toString()).isNAC());
		}
		...
		private int getConst(Operand op) {
			...
			if (op instanceof RegisterOperand) {
				SingleCP o = val.get(((RegisterOperand) op).getRegister()
						.toString());
				if (o.state == 1)
					return o.getConst();
			}
